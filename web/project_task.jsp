            ...
            for (i = 0; i < jsProject.tasks.length; i++) {
                if (jsProject.tasks[i].parentId === task.id) {
                    if (role === '46' || role === '6' || role === '45') {
                        ...
                        "<td><button id=\"DeleteTaskBtn\" class=\"btn-danger btn\">Delete</button></td></tr>");
                    } else {
                        $('#taskBody').append("<tr><td></td><td></td>");
                    }
                    $('#taskBody').append("<td><a style=\"cursor:pointer\" href=\"#\" onclick=\"updateTask(" + jsProject.tasks[i].id + ")\"><b>" + jsProject.tasks[i].name + "</b></a></td><td>" + jsProject.tasks[i].createdby.fullNames + "</td><td>" +
                ...
                $("#dailyUpdateForm").submit(function (event) {
                    ...
                    $.ajax({
                        ...
                        data: JSON.stringify({projectId: jsProject.id,
                            ...
                            updateDate: $('#dailyUpdateDate').val()
                        }),
                        dataType: 'json',
                        success: function (data) {
                ...
                for (k = 0; k < project.tasks.length; k++) {
                    if ((project.tasks[k].id === task.id) || (project.tasks[k].parentId === task.id)) {
                        ...
                        $.each(JSON.parse(convertedComments), function (idx, cmnt) {
                            var object = {"id": cmnt.id, "parent": cmnt.parentId, "created": cmnt.creationDate, "modified": cmnt.updateDate,
